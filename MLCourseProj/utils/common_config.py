...
def get_model(p, pretrain_path=None):
    # Get backbone
    if p['backbone'] == 'resnet18':
        if p['train_db_name'] in ['cifar-10', 'cifar-20', 'my_data']:
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    elif p['train_db_name'] == 'my_data':
        from data.my_data import my_data_train
        dataset = my_data_train()
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False):
    ...
    elif p['val_db_name'] == 'my_data':
        from data.my_data import my_data_val
        dataset = my_data_val()
